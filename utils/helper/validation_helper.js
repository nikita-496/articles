...
const validateName = async (validationName, res, field) => {
  if (validationName.length > 50) {
    return res.status(400).json({
      error:
        field === 'Имя'
          ...
          : 'Фамилия должна быть не более 50 символов',
    });
    ...
    return res.status(400).json({
      error:
        field === 'Имя'
          ? 'Имя должно состоять из букв латинского алфавита или кириллицы'
          : 'Фамилия должна состоять из букв латинского алфавита или кириллицы',
    });
...
const validateLogin = async (validationLogin, res) => {
  if (validationLogin.length > 50) {
    ...
    return res.status(400).json({
      error:
        'Логин не должен содержать пробелы, специальные символы: !, @, #, $, %, :, ^, &, *, ?, /, =, +, -, (, ), ;, №',
    });
  }
